...
const Categories = ({ onBack }) => {
  ...
  const totalCategoryValue = safeCategories.reduce(
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-200/60">
        <div>
          ...
          <p className="text-slate-500 font-medium mt-1">
            Distribusi kategori berdasarkan lowongan aktif unik yang ditampilkan
            ke user.
          </p>
        </div>
      </div>
            ...
            >
              ...
              <div className="mt-4 flex items-center justify-between">
                ...
                <div className="flex items-center gap-1 text-emerald-500 text-xs font-bold">
                  ...
                  {totalCategoryValue > 0
                    ? (
                        ((Number(cat?.value) || 0) / totalCategoryValue) *
                        100
                      ).toFixed(1)
                    : "0.0"}
                  %
              ...
              <div className="mt-6 w-full bg-slate-50 h-2 rounded-full overflow-hidden">
                <Motion.div
                  initial={{ width: 0 }}
                  animate={{
                    width: `${totalCategoryValue > 0 ? ((Number(cat?.value) || 0) / totalCategoryValue) * 100 : 0}%`,
                  }}
